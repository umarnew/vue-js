...
<div class="container-fluid" style="margin-top:10px">
    ...
    <div class="counter">
        <p style="text-align:center; font-size:18px">Lorem ipsum dolor sit amet consectetur adipisicing elit. Doloremque molestiae asperiores ab soluta mollitia totam nam, sapiente atque ut nesciunt sint qui explicabo possimus earum quis laboriosam. Minima praesentium vitae impedit quos. Voluptatum labore perspiciatis iste eaque aliquam dolorem veniam cumque, explicabo illum aliquid, quidem expedita eligendi qui molestias laborum!</p>
        <svg :viewBox="`0 0 ${UNITS} ${UNITS}`" :height="size || '10%'" :width="size || '10%'" style="margin-right:100px">
            <g :transform="`rotate(${rotate},${UNITS / 2},${UNITS / 2}) ${
                reverse ? 'scale(1,-1) translate(0, -' + UNITS + ')' : ''
            }`">
                <circle :cx="UNITS / 2" :cy="UNITS / 2" :r="getRadius()" :stroke="stroke" :stroke-width="getStrokeWidth(strokeWidth)" fill="none" :stroke-dasharray="getLengths()" />
                <path :d="describeArc(UNITS / 2, UNITS / 2, getRadius(), 0, activeEnd())" fill="none" :stroke="activeStroke" :stroke-width="getStrokeWidth(activeWidth)" :stroke-dasharray="getLengths()" />
            </g>
        ...
        <svg :viewBox="`0 0 ${UNITS} ${UNITS}`" :height="size || '10%'" :width="size || '10%'" >
            <g :transform="`rotate(${rotate},${UNITS / 2},${UNITS / 2}) ${
                reverse ? 'scale(1,-1) translate(0, -' + UNITS + ')' : ''
            }`">
                <circle :cx="UNITS / 2" :cy="UNITS / 2" :r="getRadius()" :stroke="stroke" :stroke-width="getStrokeWidth(strokeWidth)" fill="none" :stroke-dasharray="getLengths()" />
